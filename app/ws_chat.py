...
from app.search_probe import should_probe_search
...
def _contains_search_command(text: str | None) -> bool:
    if not text:
        return False
    ...
    try:
        while True:
            ...
            try:
                ...
                expand_id = None
                probe_text = None
                if should_probe_search(request.prompt):
                    probe_request = GenerateRequest(**request_data)
                    setattr(probe_request, "_disable_thinking", True)
                    search_enforcer = (
                        "\n\n### CRITICAL INSTRUCTION:\n"
                        "Only output [SEARCH: <query>] if the question is time-sensitive "
                        "(today/latest/current/recent/news/scores/prices/schedules) or explicitly "
                        "requests browsing.\n"
                        "Do NOT search for general biographies or timeless facts.\n"
                        "If no search is needed, answer directly without emitting [SEARCH: ...]."
                    )
                ...
                if not search_query and not expand_id:
                    ...
                    if probe_text is None:
                        no_search_enforcer = (
                            "\n\n### SEARCH POLICY:\n"
                            "Do NOT use [SEARCH] for this request. Answer from internal knowledge "
                            "unless the user explicitly asks to browse or requests time-sensitive information."
                        )
                        request.system_prompt = base_system + no_search_enforcer
                        direct_result = await engine.generate_text(request)
                        direct_text = direct_result.get("text", "")
                        if _contains_search_command(direct_text):
                            hard_block = (
                                "\n\n### CRITICAL:\n"
                                "Under no circumstances emit [SEARCH]. Provide a direct answer now."
                            )
                            request.system_prompt = base_system + hard_block
                            direct_result = await engine.generate_text(request)
                            direct_text = direct_result.get("text", "")
                        await websocket.send_json({"type": "token", "content": direct_text})
                        await websocket.send_json({"type": "end", "content": ""})
                        if session_id:
                            await append_session_message(session_id, "assistant", direct_text)
                        continue
